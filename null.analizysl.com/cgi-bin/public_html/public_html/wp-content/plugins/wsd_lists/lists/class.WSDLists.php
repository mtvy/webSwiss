...
include_once 'class.WSDListsFields.php';
include_once 'class.WSDListsItem.php';

define ('WSD_LISTS_PARENT','curiers');
if(!defined('WSD_LISTS_PARENT'))define ('WSD_LISTS_PARENT','wsdlists');

class WSDLists {

//    public $object = null;
//    public $user = null;
    public $page = 'wsdlists';//lab_groups
    public $parent_page = WSD_LISTS_PARENT;
    public $use_as_parent = false;
    public $table_name = 'wsd_lists';
    public $chengeBlocked = !true;
    public $menu_title = 'Списки sets';
    public $menu_name = 'Списки sets';
...
//        add_filter( 'manage_users_custom_column', [$this,'manage_users_custom_column'], 10, 3 );
    ...
    //            $set = [];
    //            $set['page'] = $list['page'];
    //            $set['parent_page'] = $list['parent'];
    //            $set['use_as_parent'] = $list['asparent'];
    //            $set['table_name'] = $list['table'];
    //            $set['chengeBlocked'] = $list['nochenge'];
    //            $set['menu_title'] = $list['menu_title'];
    //            $set['menu_name'] = $list['menu_name'];
                $ol = new WSDList($list); // object list
                add_action( 'init', array( $ol, 'init' ) );
...
//            $pf = new DSopProductField();
            $pf = new WSDListsItem($this->table_name);
            submit_button();
        }
        if ($active_tab == 'add_field') {
            ...
            settings_fields($this->page); // меняем под себя только здесь
//            $pf = new DSopProductField();
            $pf = new WSDListsFields($this->table_name);
            submit_button();
        }
        if ($active_tab == 'edit') {
            if ($fid) {
...
//                $form_action = "users.php?page=" . 'lab_groups';
        ...
        if ($active_tab == 'edit_field') {
            if ($fid) {
                ...
                //            $this->build_edit();
//                $pf = new DSopProductField();
                $pf = new WSDListsFields($this->table_name); // MedLabLabGroupFields();
                submit_button();
            } else {
//                $r_page = $this->rpage;
//                $form_action = "edit.php?post_type={$this->name}&page=".$r_page.'_fields';
...
//        showLogInfo('admin');
        ...
        /**
         * иконки:
         * https://developer.wordpress.org/resource/dashicons/#universal-access-alt
         */
        public function options() {

            if($this->use_as_parent){
                $hookname = add_menu_page( $this->menu_title, $this->menu_name, 'manage_options',
                    $this->page, [$this, 'page_wrapper'],'dashicons-location-alt');
            }else{
            
                $hookname = add_submenu_page( $this->parent_page,
        //        $hookname = add_submenu_page( "users.php",
                    $this->menu_title, $this->menu_name, 'manage_options',$this->page, [$this,'page_wrapper']);
            }
...
//                    $this->_notice( '<div> 0 </div>');
                ...
                if ($ft == 'remove_field') {
                    ...
                    if ($isok == 'ok') {
//                    $this->_notice( '<div> 0 </div>');
                        WSDListsFields::removeField($fid);
                    }
                }
            }
            global $lt, $lft;
...
//        $active_tab = isset($_GET['tab']) ? $_GET['tab'] : 'list';
        $active_tab = filter_has_var(INPUT_GET, 'tab') ? filter_input(INPUT_GET, 'tab', FILTER_SANITIZE_STRING) : 'list';
        if ($active_tab == 'list') {
            $ltf = 'class.WSDListsListTable.php';
            require_once $ltf;
            $lt = new WSDListsListTable($this->table_name,"admin.php?page=" . $this->page);
        }
        if ($active_tab == 'list_fields') {
            $ltf = 'class.WSDListsFieldsTable.php';
            require_once $ltf;
            $lft = new WSDListsFieldsTable($this->table_name,"admin.php?page=" . $this->page . '&tab=list_fields');
        }
//        $r_page = $this->rpage;
//        $lt = new DShopProductListTable("edit.php?post_type={$this->name}&page=".$r_page.'_fields');
//            $lt = new WSDListsListTable("admin.php?page=" . 'lab_groups');
        }
    public function shortcode_reserve($atts,$content,$tag){
        return $this->shr__dshop($tag,'');
    }
