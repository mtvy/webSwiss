...
//        $pass = 'P5x8w934JZn62a7F';
//        $sender = '50027';
//        $sender = get_option('medlab_connect_sender');
//        $pass = get_option('medlab_connect_pass');
...
BirthYear int (необязательно) Год рождения пациента (если полная дата неизвестна).
        ...
        $fields['DocumentNumber'] = false;
        $fields['id_citizen'] = false;
        $fields['BodyTemperature'] = false;
        $fields['Adres'] = false;
        $fields['eComment'] = false;
        $fields['cardNr'] = false;
        
        $fields['PayCategoryId'] = false;
        $fields['paymentmethod_sl'] = false;
...
//            $fields['BirthYear'] = get_user_meta( $uid, 'born_year', true );
            $fields['DocumentNumber'] = get_user_meta( $uid, 'passnum', true );
            $fields['id_citizen'] = get_user_meta( $uid, 'id_citizen', true );
            $fields['BodyTemperature'] = get_user_meta( $uid, 'temperature', true );
            $fields['Adress'] = get_user_meta( $uid, 'residence_place', true );
            $fields['eComment'] = get_post_meta( $orderId, 'dso_q_ref_comment', true );
            $fields['eComment'] = get_user_meta( $uid, 'pat_comment', true );
            $fields['Comment'] = get_user_meta( $uid, 'pat_comment', true );
            $fields['comment'] = get_user_meta( $uid, 'pat_comment', true );
            $fields['Comment1'] = get_user_meta( $uid, 'pat_comment', true );
            $fields['comment1'] = get_user_meta( $uid, 'pat_comment', true );
            $fields['cardNr'] = get_user_meta( $uid, 'card_numer', true );
//            add_Log($fields);
        }
        ...
        if(isset($args['dso_id'])){
            ...
            $discontID = get_post_meta( $orderId, 'dsop_discont_id', true );
            $discontC = DSDiscont::_get_discont_code($discontID);
            $fields['PayCategoryId'] = $discontC;
            $fields['paymentmethod_sl'] = get_post_meta( $orderId, 'dsop_method_sl', true );
    ...
    public function mlrb_Orders ($args = []){
      ...
      ';
        ...
        foreach($args['orders'] as $n=>$v){
            ...
            $fields['State'] = false;
            $fields['Defected'] = false;
            $fields['Defects'] = false;
            $fields['DefectCode'] = false;
            if(isset($v['code']))$fields['Code'] = $v['code'];
            if(isset($v['BiomaterialCode']))$fields['BiomaterialCode'] = $v['BiomaterialCode'];
            if(isset($v['State']))$fields['State'] = $v['State'];
            if(isset($v['Defected']))$fields['Defected'] = $v['Defected'];
            if(isset($v['Defects']))$fields['Defects'] = $v['Defects'];
            if(isset($v['DefectCode']))$fields['DefectCode'] = $v['DefectCode'];
            $attributes = [];
            $i_sep = " ";
            $sep = "\n        ";
            foreach($fields as $n=>$v){
