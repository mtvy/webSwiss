...
//      $monthName = self::RUSSIAN_MONTHS[date('m', $date) - 1];
      $monthName = $RUSSIAN_MONTHS[date('m', $date) - 1];
    }
...
//      throw new \Exception($message);
    ...
    $ml_ip = '188.165.90.186';
    $ml_port = '9901';
    $ml_pass = 'P5x8w934JZn62a7F';
    $ml_sender = '50027';
    
    $ml_ip = get_option('medlab_connect_ip');
    $ml_port = get_option('medlab_connect_port');
    $ml_sender = get_option('medlab_connect_sender');
    $ml_pass = get_option('medlab_connect_pass');
    
    global $ml_main_ip,$ml_main_port;
    global $ml_main_pass,$ml_main_sender;
    
    $ml_main_ip = $ml_ip;
    $ml_main_port = $ml_port;
    $ml_main_pass = $ml_sender;
    $ml_main_sender = $ml_pass;
    
function restore_ml_access(){
    global $ml_ip,$ml_port;
    global $ml_pass,$ml_sender;
    global $ml_main_ip,$ml_main_port;
    global $ml_main_pass,$ml_main_sender;
    ...
    $ml_ip = $ml_main_ip;
    $ml_port = $ml_main_port;
    $ml_sender = $ml_main_pass;
    $ml_pass = $ml_main_sender;
}
    
function set_ml_access_by_group($gid=false){
    global $ml_ip,$ml_port;
    global $ml_pass,$ml_sender;
    
    $use_access = get_option_ml_group('use_access',$gid,1);
    if(!$use_access) return;
...
//    add_log($params);
    ...
    ob_start();
    $fp = fopen('http://'.$ml_ip.':'.$ml_port.'/', 'rb', FALSE, $context);
//    $fp = fopen('http://'.$ip.':'.$port.'/isys/', 'rb', FALSE, $context);
//    $fp = fopen('http://'.$ip.':'.$port.'/isys/', 'wb', FALSE, $context);
//    $fp = fopen('http://95.211.223.217:9901/', 'rb', FALSE, $context);
//    $fp = fopen('95.211.223.217:9901', 'rb', FALSE, $context);
