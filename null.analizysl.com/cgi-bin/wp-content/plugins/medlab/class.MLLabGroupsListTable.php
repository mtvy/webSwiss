...
/* 
 * class.MLLabGroupsListTable.php
 * 
 * https://wp-kama.ru/function/wp_list_table
 */

class MLLabGroupsListTable extends WP_List_Table {
	...
	function prepare_items(){
        ...
        $q= "select count(*) from `$table_ml_groups` ";
...
//        $q= "select * from `$dsp_fields` order by `weigh`";
        $q= "select * from `$table_ml_groups` order by `id`";
...
//		echo '<div class="alignleft actions">HTML код полей формы (select). Внутри тега form...</div>';
	...
	function column_default( $item, $colname ){
		...
		if( $colname === 'title' ){
			...
			$actions = array();
			$actions['edit'] = sprintf( '<a href="%s">%s</a>', $this->url_edit.'&tab=edit_lab_group&fid='.$item['id'], __('edit','hb-users') );

			$actions['trash'] = sprintf( '<a href="%s">%s</a>', $this->url_edit.'&tab=delete_lab_group&fid='.$item['id'], __('delete','hb-users') );
		...
		if( $colname === 'use_access' ){
            $en = [1=>'Да',0=>'Нет'];
			return $en[$item['use_access']];
		}
		else
		if( $colname === 'dump' ){
			return $item->$colname;
