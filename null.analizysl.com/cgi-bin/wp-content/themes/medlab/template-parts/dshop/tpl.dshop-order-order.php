...
<!--      <td nowrap><a href="/order?oid=<?=$dsorder->ID?>"
    ...
    <tr>
      ...
      <td class="<?=$state_class?>"><?=$state_f?></td>
        ...
        <?php
        $DSPsForm = "";
        if($state_ == 'checked'){ // !!! определить стстус доступный для оплаты
...
//                    $discontID = 7;
//                    add_log('-- '.$discontID);
                    $discontN = DSDiscont::_get_discont_name($discontID);
                    $discontC = DSDiscont::_get_discont_code($discontID);
                    add_log('Имеется скидка: +'.$discont.' % -- '.$discontN.' ['.$discontC.']');
//                    add_log($DSDiscont);
//                    add_log($_SESSION);
                }
...
//            $discontC = DSDiscont::_get_discont_code($discontID);
//            $PayCategoryId = $discontC;
//            $paymentmethod_sl = get_post_meta( $orderId, 'dsop_method_sl', true );
//                    add_log($PayCategoryId);
//                    add_log($paymentmethod_sl);
//                    $discontN = DSDiscont::_get_discont_name($discontID);
//                    $discontC = DSDiscont::_get_discont_code($discontID);
