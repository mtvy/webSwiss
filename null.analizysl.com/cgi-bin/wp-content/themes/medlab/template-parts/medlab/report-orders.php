...
/* 
 * 
 * [medlab tpl='report' type='orders' ]
 * report-orders.php
 */

global $ht;
//echo $ht->f('h2','report-orders');
...
foreach($hiddens as $hn=>$hv){
    $hiddensf[] = $ht->f('input','',['type'=>'hidden','name'=>$hn,'value'=>$hv])."\n";
...
//    foreach( $_posts as $dsorder ){
        ...
        $time = date( "H:i", strtotime( $dsorder->post_date ) );
        ...
        $dso_userId_ =  get_post_meta( $orderId, 'dso_userId', true );
//        $dso_luid =  get_post_meta( $orderId, 'dso_duid', true );  // lab
        $lab_group = (int) get_user_meta($dso_userId_,'lab_group',1);
//        $sel_g
        ...
        $group = '';
        $payform =  'cashe';
        if($lab_group>0)$group = $sel_g[$lab_group];
    
        $uid = $dso_puid;
        $fio = get_user_meta($uid,'last_name',1);
        $fio .= ' '.get_user_meta($uid,'first_name',1);
        $fio .= ' '.get_user_meta($uid,'second_name',1);
        $df_sex = get_user_meta($uid,'gender',1);
        $gender = $genders[$df_sex];
        $df_sex = get_user_meta($uid,'gender',1);
        $born_date = get_user_meta($uid,'born_date',1);
        $phone = get_user_meta($uid,'phone',1);
        $bdt = explode('.',$born_date);
        if(count($bdt)==3){
            $cdt = explode('.',date('d.m.Y'));
            $years = $cdt[2]-$bdt[2];

            if($cdt[0]==$bdt[0] && $cdt[1]==$bdt[1]){}
            else
            if($cdt[0]<$bdt[0] && $cdt[1]==$bdt[1]){
                $years-=1;
            }
...
$hitems[]='ФИО';
$hitems[]='Пол';
$hitems[]='Дата рождения\возраст';
$hitems[]='Контакты';
$hitems[]='Наименование Вид';
$hitems[]='Наименование Название';
$hitems[]='Пункт прием (Город, фелиал)';
//$hitems[]='Номер карты';
//$hitems[]='Номер заказа';
$hitems[]='Сумма заказа';
$hitems[]='Форма оплаты';
...
//echo $c;
    ...
    for($p=1;$p<=$pages;$p++){
        ...
        $ata = ['class'=>'page-link '.($p==$page?'bg-primary text-white':''),'href'=>get_the_permalink( get_the_ID() ) . '?pg=' . $p .'&'.$urlget];
...
?>
<div class="row ">
    <form action="<?=get_the_permalink( get_the_ID() )?>" method="post" class="col-12 text-left  border border-primary mb-2 ">
        <?php
...
<?php
if(0&& !$isdoctor && !$isagent){
?>
<form action="<?=get_the_permalink( get_the_ID() ).$p?>" method="post" class="row">
    <div class="row" >
