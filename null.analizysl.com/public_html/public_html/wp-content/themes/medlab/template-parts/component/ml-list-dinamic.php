...
//                    $out = ('<pre>'.htmlspecialchars(print_r($labGroups_,1)).'</pre>');
//                    echo DShop::_div(DShop::_div(DShop::_div($out,'border border-primary mb-2'),'col-12'),'row');
...
//        https://getbootstrap.com/docs/4.0/components/alerts/
        ...
        $xml_res=[];
        foreach ($results as $key => $item) { // набор заявок пункта
...
//            echo DShop::_div(DShop::_div(DShop::_div($out,'border border-primary mb-2'),'col-12'),'row');
            ...
            $queryStatus .= DShop::_div(DShop::_div(DShop::_div(
                    'Заявка: '.$item_['LisId'].'. '.
                    'Пункт: '.$labTitle.'. '.
...
//            $tab_rows [] =  [['colspan'=>5,'val'=>'<hr/> '.$rowBlokInfo]];
            
            $Date = $Date.'_'.$labGroup['id'];
...
//            echo  DShop::_div(DShop::_div(DShop::_div($ht->pre(print_r($Date,1)),'alert alert-warning'),'col-12'),'row');
...
//                    $out = ('<pre>'.htmlspecialchars(print_r($orders[$order]['Tests'][$f_GrCode]['Tests'][$f_tests],1)).'</pre>');
    ...
    $researches .= DShop::_div(DShop::_div(DShop::_div($out,'border border-primary mb-2'),'col-12'),'row');
}
...
//            echo  DShop::_div(DShop::_div(DShop::_div(dechex(136),'alert alert-warning'),'col-12'),'row');
            ...
            $i = 0;
//            for($i=0;$i<=3;$i++){
//            echo  DShop::_div(DShop::_div(DShop::_div($ht->pre(print_r($test,1)),'alert alert-warning'),'col-12'),'row');
...
//                    if($labGroup == 5)$key[0]-=2;
//                    if($labGroup == 20)$key[0]-=1;
                    $chDay =  (mktime( 0,0,0,$key[1],$key[0],$key[2]) ."000"); // +(1000*3600*24*$i*$i);
                    if($chstart)
...
//    $tab_rows [] =  [['colspan'=>5,'val'=>'<hr/> ']];
    echo DShop::_div(DShop::_div(DShop::_div('Количество результатов: '.$answer_count_.' ','alert alert-primary'),'col-12'),'row');
    echo $queryStatus;
    echo $researches;
