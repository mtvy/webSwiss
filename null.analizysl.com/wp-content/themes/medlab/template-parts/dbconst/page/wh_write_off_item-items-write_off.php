...
/* 
 * wh_write_off_item-items-write_off.php
 * main template
 */

//echo 'weybill_item-items-weybillget';
//add_log($this->data);
...
//        $col.=$it[$field].' ';
        if($field == 'id'){
            ...
            $wh_item_id = $it['material_id_id'];
            $wh_item_id = str_pad($wh_item_id,20,'0',STR_PAD_LEFT);
//            $barcode = MLBarcode::img($dso_query_nr);
            $barcode = MLBarcode::img_file($wh_item_id, WAREHOUSEUPLOADS,'wh_item',$bc_attr);
            $col.=$id.'<br/> '.$barcode;
        }
        else
//        if(in_array($field,$fields))$col.=$it[$field].' '.$this->form_field($field,'',0,$it);
        if(in_array($field,$fields))$col.=$this->form_field($field,'',0,$it);
        else $col.=$it[$field];
        $item[]=$col;
      }
...
//              .'</div><div class="col-1"></div><div class="col-11">' . $list_vars['status_deliv'][$it['status_deliv']]
              ;
      if(0)$item[]= ['val'=>$tools,'class'=>'col-12 text-right'];
