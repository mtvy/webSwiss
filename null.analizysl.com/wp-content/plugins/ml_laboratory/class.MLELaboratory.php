<?php
/*
 * class.MLELaboratory.php
 */


include_once 'class.MLELaboratoryFields.php';
include_once 'class.MLELaboratoryItem.php';

class MLELaboratory {

    public $object = null;
    public $user = null;
    public $page = 'laboratories';//lab_groups
    public $table_name = 'laboratory';
...
//        $form_action = "users.php?page=".'lab_groups';
        ...
        $form_action = "admin.php?page=" . $this->page;//'curiers';
...
//            $pf = new DSopProductField();
            $pf = new MLELaboratoryItem($this->table_name);
            submit_button();
        }
        if ($active_tab == 'add_field') {
            ...
            settings_fields($this->page); // меняем под себя только здесь
//            $pf = new DSopProductField();
            $pf = new MLELaboratoryFields($this->table_name);
            submit_button();
        }
        if ($active_tab == 'edit') {
            if ($fid) {
...
//                $form_action = "users.php?page=" . 'lab_groups';
        ...
        if ($active_tab == 'edit_field') {
            if ($fid) {
                ...
                //            $this->build_edit();
//                $pf = new DSopProductField();
                $pf = new MLELaboratoryFields($this->table_name); // MedLabLabGroupFields();
                submit_button();
            } else {
//                $r_page = $this->rpage;
//                $form_action = "edit.php?post_type={$this->name}&page=".$r_page.'_fields';
...
//            $hookname = add_menu_page('Лаобратрии', 'Лаборатории', 'manage_options',
...
//            ''.$r_page.'_fields', [$this,'page_wrapper']);
        $hookname = add_submenu_page( 'curiers',
//        $hookname = add_submenu_page( "users.php",
                'Лаобратрии', 'Лаобратрии', 'manage_options',$this->page, [$this,'page_wrapper']);
...
//                    $this->_notice( '<div> 0 </div>');
                        MLELaboratoryItem::removeField($fid);
                    }
                }
                if ($ft == 'remove_field') {
                    ...
                    if ($isok == 'ok') {
//                    $this->_notice( '<div> 0 </div>');
                        MLELaboratoryFields::removeField($fid);
                    }
                }
            }
            global $lt, $lft;
        ...
        $active_tab = isset($_GET['tab']) ? $_GET['tab'] : 'list';
        if ($active_tab == 'list') {
            $ltf = 'class.MLELaboratoryListTable.php';
            require_once $ltf;
            $lt = new MLELaboratoryListTable($this->table_name,"admin.php?page=" . $this->page);
        }
        if ($active_tab == 'list_fields') {
            $ltf = 'class.MLELaboratoryFieldsTable.php';
            require_once $ltf;
            $lft = new MLELaboratoryFieldsTable($this->table_name,"admin.php?page=" . $this->page . '&tab=list_fields');
        }
//        $r_page = $this->rpage;
//        $lt = new DShopProductListTable("edit.php?post_type={$this->name}&page=".$r_page.'_fields');
//            $lt = new MLELaboratoryListTable("admin.php?page=" . 'lab_groups');
        }
