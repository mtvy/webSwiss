...
//        add_action('save_post', 'true_save_box_data');
    ...
    public function post_type_init() {
        $labels = array(
            ...
            'view_item' => 'Просмотр Заказ на сайте',
...
//                + array_slice( $columns, $num );
        ...
        if( array_key_exists('dso_userId',$this->meta_fields)  ){
		$num = 2; // после какой по счету колонки вставлять новые
		$new_columns = array( 'dso_userId' => 'Создал' );
        $columns = array_slice( $columns, 0, $num ) + $new_columns
                + array_slice( $columns, $num );
        }
        
        if( array_key_exists('dso_discont',$this->meta_fields)  ){
		$num = 2; // после какой по счету колонки вставлять новые
		$new_columns = array( 'dso_discont' => 'Скидка' );
        $columns = array_slice( $columns, 0, $num ) + $new_columns
                + array_slice( $columns, $num );
        }
    ...
    //            'foreign'=>'Иностранные',
        ...
        if($colname == 'dso_status'){
            ...
            $state_class = apply_filters('ds_dsorder_post_display_meta_box_order__class_out_status', $state_class, $d, $dsorder, $this);
...
//                }
        ...
        if($colname == 'dso_userId'){
            global $ht;
//            https://null.analizysl.com/wp-admin/user-edit.php?user_id=696&wp_http_referer=%2Fwp-admin%2Fusers.php
            $d=get_post_meta( $post_id, $colname, 1 );
            if(( strlen(trim($d)) > 0 )){
                $state_class = '';
                if($d){
...
//                    $atr[''] = '';
                    $atr['href'] = '';
//                    rawurlencode()
                    $atr['href'] = 'https://analizysl.com/wp-admin/user-edit.php?user_id=696&wp_http_referer=%2Fwp-admin%2Fusers.php';
                    $atr['href'] = 'user-edit.php?user_id='.$d.'&wp_http_referer=%2Fwp-admin%2Fedit.php%3Fpost_type=dsorder';
                    $atr['href'] = 'user-edit.php?user_id='.$d.'&wp_http_referer='.rawurlencode('/wp-admin/edit.php?post_type=dsorder');
                    $d = $ht->f('a',$d,$atr);
                }
//                $currency_short = get_option('currency_short','zl');
...
//        high, core, default или low
        ...
        foreach($this->meta_name as $name=>$title){
            do_action('ds_dsorder_post_display_meta_box__add_onpos', $this,$this->name,$num++);
        ...
        do_action('ds_dsorder_post_display_meta_box__add', $this,$this->name);
    }
    /*
     * Этап 2. Заполнение
     */
...
//        $dso_markup_ =  get_post_meta( $dsorder->ID, 'dso_markup', true );
        ...
        $dso_cost_ =  get_post_meta( $dsorder->ID, 'dso_cost', true );
        $dso_count_ =  get_post_meta( $dsorder->ID, 'dso_count', true );
        
        $dso_discont_ =  get_post_meta( $dsorder->ID, 'dso_discont', true );
        $dso_total_ =  get_post_meta( $dsorder->ID, 'dso_total', true );
        $dso_payd_ =  (int)get_post_meta( $dsorder->ID, 'dso_payd', true );
        
        $dso_total_ = $dso_cost_;
        
        /* MedLab */
        $dso_arate = (int) get_post_meta( $dsorder->ID, 'dso_agent_rate', true );
        $dso_drate = (int) get_post_meta( $dsorder->ID, 'dso_doctor_rate', true );
        $dso_dbonus_state = (int) get_post_meta( $dsorder->ID, 'dso_dbonus_state', true );
        $dso_agent_percent_ = $dso_arate;
        $dso_doctor_percent_ = $dso_drate;
        
        $dso_agent_sum_ = ($dso_payd_/100) * $dso_arate;
        $dso_doctor_sum_ = ($dso_payd_/100) * $dso_drate;
        $dso_lab_sum_ = $dso_payd_ - $dso_agent_sum_;
...
//        $fields=$this->meta_fields;
        ...
        <table>
            <tr>
                <td style="width: 100px" colspan="2">Состояние заказа</td>
            </tr>
            <tr>
                <td colspan="2">
...
//                    for ( $rating = 5; $rating >= 1; $rating -- ) {
                        if(is_array($name)){
                            ...
                            foreach ( $name as $v=>$t) {
                                echo '<option value="'.$v.'" '.selected( $v, $state_, 0 ).'>'.$t."\n";
                            }
                            echo '</optgroup>';
                        }else{
                    ?>
                        <option value="<?php echo $state; ?>" <?php echo
                        selected( $state, $state_  , 0); ?>><?php echo $name; ?>  <?php
                        }
                    }?>
                    </select>
                </td>
            ...
            <tr><td style="" colspan="1">Количество позиций</td>
                <td colspan="1"><?=$dso_count_?></td></tr>
            <tr><td style="" colspan="1">Количество единиц</td>
                <td colspan="1"><?=$dso_items_count_?></td></tr>
            
            <tr><td style="" colspan="2"><b>Итог:</b></td></tr>
            
            <tr><td style="" colspan="2">Стоимость</td></tr>
            <tr><td colspan="2"><input style="width: 80%" type="text"
                name="dso_cost" value="<?=$dso_cost_?>"></td></tr>
            
            <tr><td style="" colspan="2">Скидка</td></tr>
            <tr><td colspan="2"><input style="width: 80%" type="text"
                name="dso_discont" value="<?=$dso_discont_?>">%</td></tr>
            
            <tr><td style="" colspan="2">Общая стоимость</td></tr>
            <tr><td colspan="2"><input style="width: 80%" type="text"
                name="dso_total" value="<?=$dso_total_?>"></td></tr>
            
            <tr><td style="" colspan="2">Оплачео</td></tr>
            <tr><td colspan="2"><input style="width: 80%" type="text"
                name="dso_payd" value="<?=$dso_payd_?>"></td></tr>
            
            <!-- MedLab -->
            <tr><td style="" colspan="2"><b>Расход:</b></td></tr>
            
            <tr><td style="" colspan="1">Процент представителя</td>
                <td colspan="1"><?=$dso_agent_percent_?></td></tr>
            <tr><td style="" colspan="1">Процент врача</td>
                <td colspan="1"><?=$dso_doctor_percent_?></td></tr>
            
            <tr><td style="" colspan="1">Оплата представителя</td>
                <td colspan="1"><?=$dso_agent_sum_?></td></tr>
            <tr><td style="" colspan="1">Оплата врача</td>
                <td colspan="1"><?=$dso_doctor_sum_?></td></tr>
            <tr><td style="" colspan="1">Доход лаборатории</td>
                <td colspan="1"><?=$dso_lab_sum_?></td></tr>
            <tr><td style="" colspan="2"><b>Бонус: <?=$dso_dbonus_state?'':'не'?> выплачен</b></td></tr>
...
//        do_action_ref_array($tag, $args);
        do_action('ds_dsorder_post_display_meta_box_user__fields_out', $this,$dsorder, $meta);
        ?>
        </table>
        <?php
    }
...
//        $v_=get_option($option_name,'desctop');
     ...
     * @param int $orderId post_id
     * @param object $dsorder
     */
    public function add_metabox_fields( $orderId, $dsorder ) {
		
        /*
         * ===== ===== ===== ===== ===== 
         * insert telegramm messages
         * start
         * ===== ===== ===== ===== ===== 
         */
        
		$user = get_user_by('ID', $dsorder->post_author);
		$telegramChatID = get_user_meta($dsorder->post_author, 'wptelegram_login_user_id', true);
		$first_name = get_user_meta($user->ID, 'first_name', true);
		$last_name = get_user_meta($user->ID, 'last_name', true);
		$botToken = get_option('wptelegram_login');
		
		if($botToken == true and $telegramChatID == true){
			
			if(@$_POST['form-type'] == 'create_order'){
				
				$message = '<b>Здравствуйте, уважаемый '.$first_name.' '.$last_name.'.</b>'.PHP_EOL.'Ваш заказ # '.$orderId.' был создан.'.PHP_EOL.'Когда произойдет изменения статуса вы получите уведомление.';
					
				$urlToSend = 'https://api.telegram.org/bot'.$botToken['bot_token'].'/sendMessage?chat_id='.$telegramChatID.'&text='.urlencode($message).'&parse_mode=HTML';
					
				file_get_contents($urlToSend);
				
			}else{
				
				$newStatus = (@$_POST['dso_status']) ? $_POST['dso_status']:'undefined';
				...
				if($telegramChatID == true){
					
					$message = '<b>Здравствуйте, уважаемый '.$first_name.' '.$last_name.'.</b>'.PHP_EOL.'Ваш заказ # '.$orderId.' был обновлен.'.PHP_EOL.'Текущий статус вашего заказа: '.$states[$newStatus].'';
        ...
        do_action('ds_dsorder_post__save', $this, $orderId, $dsorder);
        }
    }
}